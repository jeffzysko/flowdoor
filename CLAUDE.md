...
10. **A foto sai carimbada** com data, hora, ponto e coordenada, desenhadas na
   própria imagem em `src/lib/field/camera.ts`. Quem prova contra fraude é o
   registro do servidor. O carimbo existe porque a foto sai do sistema por
   WhatsApp e PDF, e fora daqui precisa se explicar sozinha.
...
12. **Antifraude é camada, não muro.** Coordenada de GPS é falsificável num
   ...
   - **O pHash ignora a faixa do carimbo.** A geometria mora em
     ...
     divergirem, o hash passa a medir o carimbo (igual em toda foto) e tudo
     vira duplicata.
...
14. **A chegada é uma porta, não um carimbo.** Fora do raio, `field_start`
   ...
   - **Melhor leitura, não a última.** A primeira leitura do GPS vem com
     precisão de centenas de metros e melhora em 15 a 40 s. `Execution.tsx`
   ...
   - **Permissão pedida no `/campo`, não no ponto.** `PermissaoLocalizacao`
     ...
     caminho **no sistema operacional** dela, porque o conserto é fora do
     navegador.
...
17. **Duas tarefas rodam sozinhas, no banco.** `pg_cron` está instalado e os
   ...
   **Horário do cron é UTC.** Porto Alegre é UTC−3.
   ...
   garante isso, e os `insert ... on conflict` dependem dele.
...
35. **Estado vazio tem três partes: o que falta, por quê, e o botão que
   resolve.** Quem não tem permissão para resolver não vê botão, e sim a
   frase dizendo a quem pedir.
...
87. **A unidade de venda é da FACE, não do sistema.** A decisão nº 28 dizia
   ...
   que se vendem por mês no Brasil. Uma campanha de um mês em front light era
   cobrada como três períodos de 14 dias, errado para mais e de um jeito que
   só aparece na conferência da fatura. `faces.sale_unit` diz o que
...
81. **A importação só cria e atualiza. Nunca apaga.** Planilha que chega sem
   ...
   operação". A diferença entre as duas leituras é um inventário inteiro.
...
77. **A agência acompanha a campanha por função, nunca por policy em
   ...
   GPS, identificador do aplicador e nota de comportamento, ou seja,
   rastreamento de trabalhador. A agência precisa saber que a face foi
   aplicada no dia 12; não precisa saber onde o aplicador estava às 9h14.
   `partner_order_progress()` devolve face, endereço, data e situação, e mais
   nada.
...
79. **Um fator, não uma segunda tabela.** `org_relationships.price_factor`
   ...
   sincronizado com o primeiro, e os dois divergem sem ninguém perceber.
...
72. **A disponibilidade mostra doze colunas e um ano de dados.** "Meu cliente
   ...
   respondia, porque começava sempre em hoje. O seletor de mês move a janela; o
...
52. **Opção não bloqueia a face, e isso é a decisão inteira.** O índice de
   exclusão já ignorava `kind = 'opcao'` desde o início, e agora tem tela para
...
53. **A opção tem numeração própria (`OPC-`), separada da de pedido.** Opção
   que morre não pode furar a sequência de pedidos emitidos, porque
   contabilidade não gosta de buraco. `hold_sequences` é uma tabela à parte
   de `order_sequences`.
54. **Confirmar opção chama `create_order_with_items`, não reimplementa.** O
   ...
   `consumida` depois, não antes, para o índice de exclusão só olhar para o
   pedido novo.
...
51. **Escolher face é lista aberta; `<select>` com trezentas faces é
   ...
   agendamento só existe para a face que já entrou no pedido, sem linha vazia
   esperando escolha. "Definir para todas" preenche só o que está
...
27. **A confiança na coordenada é informação de tela, não detalhe técnico.**
   ...
   precisa saber se veio do Places ou do Geocoding, e sim se pode mandar
   alguém para lá. Coluna "Local" na lista do inventário, bloco
...
21. **A coordenada é produzida pelo sistema, e carrega o quanto vale.** O
   ...
   **só `exata`, `confirmada` e `manual` armam a trava de chegada**
   (`field_start` consulta `site_lock_ready()`). Barrar um aplicador com base
   num centroide de rodovia é abrir chamado por erro nosso. Coordenada fraca
...
5. **RLS é a fronteira, não o filtro do cliente.** As policies usam
   `readable_org_ids()`, `has_org_role()` e `is_org_member()`, todas SECURITY
   DEFINER, estáveis, com `search_path` fixo. Filtro por `org_id` no front é
   conveniência.
...
8. **Campo funciona offline.** `src/lib/field/queue.ts` grava em IndexedDB antes
   ...
   duplica, porque o banco checa `field_sync_log.idempotency_key`.
...
VM**. Mas `npx tsc --noEmit` roda, porque TypeScript é JS puro, sem binário
nativo.
...
- `revoke_invitation(id)` cancela um convite pendente. E-mail digitado errado
  é um link válido por 14 dias e precisa de desligamento. Não apaga a linha:
...
- `/recuperar-senha` precisa estar em `PUBLIC_PREFIXES` no
  `src/lib/supabase/middleware.ts`, porque quem pede o link não tem sessão.
...
- Auditoria por amostragem: uma fatia das aplicações roteada para uma segunda
  pessoa conferir em campo. É processo, não software, e é o único controle
