...
begin
  ...
  if new.org_id is null then
    raise exception 'site % nao encontrado', new.site_id;
  end if;
  return new;
end;
$$;
...
begin
  ...
  if not found then raise exception 'face % nao encontrada', new.face_id; end if;
  ...
  if f.medium = 'estatico' then
    ...
    if (
      ...
      raise exception 'loop cheio: face % nao tem % spots livres no periodo', new.face_id, new.slots;
    end if;
  end if;
  return new;
end;
